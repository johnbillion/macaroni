...
pub trait ReportStore: Send + Sync {
    ...
    fn set_triage(&self, id: &str, summary: &str, validity: Option<&str>) -> AppResult<()>;
...
impl ReportStore for SqliteStore {
    ...
    fn get_triage(&self, id: &str) -> AppResult<Option<TriageRecord>> {
        let c = self.conn.lock().unwrap();
        let result: rusqlite::Result<(Option<String>, Option<String>)> = c.query_row(
            "SELECT triage, triage_validity FROM reports WHERE id = ?1",
            params![id],
            |row| Ok((row.get::<_, Option<String>>(0)?, row.get::<_, Option<String>>(1)?)),
        );
        match result {
            Ok((Some(summary), validity)) => Ok(Some(TriageRecord { summary, validity })),
            Ok((None, _)) => Ok(None),
    ...
    fn set_triage(&self, id: &str, summary: &str, validity: Option<&str>) -> AppResult<()> {
        let c = self.conn.lock().unwrap();
        c.execute(
            "INSERT INTO reports (id, read, triage, triage_validity) VALUES (?1, 0, ?2, ?3)
             ON CONFLICT(id) DO UPDATE SET triage = excluded.triage,
                                           triage_validity = excluded.triage_validity",
            params![id, summary, validity],
        )
        .map_err(AppError::other)?;
        Ok(())
    }
