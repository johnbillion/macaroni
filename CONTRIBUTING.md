...
The app is driven primarily by a local SQLite database (`<app_data_dir>/macaroni.db`), not live HackerOne API calls. The database is encrypted at rest with SQLCipher.

To inspect the database during development, use the `sqlcipher` CLI (`brew install sqlcipher`) and key it from the keychain:

```sh
sqlcipher "$HOME/Library/Application Support/com.johnbillion.macaroni/macaroni.db" \
  "PRAGMA key = \"x'$(security find-generic-password -s com.johnbillion.macaroni -a default -w | jq -r .db_key)'\";" \
...
The discussion view (the second tab in the top bar) is a second read of the same mirror: `query_comments` (`recent_comments`) joins `json_each(detail_json, '$.activities')` against the reports table, keeps the `comment` activities, and returns the newest 100 across every report in every state. It stores and fetches nothing of its own — a report shows up in it once its detail has synced, and hackbot's pre-submission trigger comments are filtered out to match what the detail pane hides. Every sidebar facet filters the inbox query, so the sidebar is hidden along with the inbox table. While the view is open the frontend holds off the sync and the inbox re-query, but not the selected-report poll — a bulk selection holds both (`listRefreshHeld` vs `detailRefreshHeld` in `App.tsx`). Because only the selected report is polled, every other report's in-state detail drifts behind the mirror; clicking a comment the held snapshot predates re-reads that report's detail from the mirror so the thread has the comment to scroll to.
